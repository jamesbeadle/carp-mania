...
3. **Game screens** — market as the tackle shop, world as the signpost, groundworks with a finger, the noticeboard, the lodge with the bailiff, setup as a story. *(built: `game-screens` — a place banner under the live sky on every deep screen, with a vignette of the place)*
4. **Speed and lists** — parallel and streamed loads with skeletons, preloading, paging and filters on lakes, inbox, catches, feed and leaderboards. *(built: `speed-and-lists` — loads in parallel, slow corners streamed behind skeletons, list views in migration 0015, paging and filters on waters, anglers, the noticeboard, catch histories, the live feed and the leaderboards)*
