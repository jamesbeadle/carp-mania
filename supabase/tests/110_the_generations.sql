...
select test.assert_that(
	(select generation = 1 and name = 'Player111' and born_age between 20 and 30 and retires_at_age between 80 and 100 and retired_at is null
...
select test.assert_refused(format('select public.retire_fisherman(%L, %L)', test.player(111), 'YoungTom'), 'permission denied for function retire_fisherman');
reset role;
select public.record_catch(test.player(111), :'visit', :'first_fish', 'The Peg', 'hair rig', 'boilie', 6, 2, 2, 2, 2, 'bankside_basics-rod-2.75-12', 'bankside_basics-reel-carp_large', 'meadowmill-bait-shelf_life_boilie') as first_catch \gset
...
select test.assert_refused(format('select public.retire_fisherman(%L, %L)', test.player(111), 'YoungTom'), 'years in him yet');
...
select test.assert_refused(format('select public.retire_fisherman(%L, %L)', test.player(111), 'T'), 'letters and numbers');
select public.retire_fisherman(test.player(111), 'YoungTom') as heir \gset
...
select test.assert_that(
	(select generation = 2 and name = 'YoungTom' and retired_at is null from public.fishermen where id = :'heir'),
	'the heir is the second of the line'
);
select test.assert_that(
	(select current_fisherman_id = :'heir' and display_name = 'YoungTom' and line_selection = 40 and watercraft = 40 and experience = 0 from public.profiles where id = test.player(111)),
	'the heir takes the name, starts at 40 and has landed nothing'
);
select test.assert_that((select angler_name = 'Player111' from public.catches where id = :'first_catch'), 'the old catch keeps the old name');
...
select public.record_catch(test.player(111), :'second_visit', :'second_fish', 'The Peg', 'hair rig', 'boilie', 6, 2, 2, 2, 2, 'bankside_basics-rod-2.75-12', 'bankside_basics-reel-carp_large', 'meadowmill-bait-shelf_life_boilie');
