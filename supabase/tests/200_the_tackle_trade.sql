...
select test.assert_that((select quantity = 300 from public.tackle_owned where profile_id = test.player(200) and item_id = 'meadowmill-bait-shelf_life_boilie'), 'and a bag of bait');
...
reset role;
select public.record_catch(test.player(200), :'visit', :'fish', 'Peg 1', 'hair_lead_clip', 'fishmeal_boilie', 4, 0, 0, 0, 0, 'marlow-rod-3-12', 'bankside_basics-reel-carp_large', 'meadowmill-bait-shelf_life_boilie') as caught \gset
...
select test.assert_that((select quantity = 299 from public.tackle_owned where profile_id = test.player(200) and item_id = 'meadowmill-bait-shelf_life_boilie'), 'a catch uses a bait');
