...
select test.give_carp(:'home', 'Stopper', 'common', 12, 90, 0) as stopper \gset
...
reset role;
select test.assert_that(test.money_of(test.player(210)) = 100000 - 300, 'the transport is paid once');
...
select test.assert_refused(format('select public.move_to_my_water(array[%L]::uuid[], %L, 300, now(), null, 10)', :'stopper', :'next_door'), 'not one of your waters');
reset role;
select test.assert_that(test.money_of(test.player(210)) = 100000 - 300, 'a refused move costs nothing');
