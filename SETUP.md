...
2. Database → Extensions: enable **pg_cron**. It closes ended auctions and ended matches every minute and purges the live feed hourly. If you enable it later, re-run `0009j_the_schedules.sql` and `0014d_closing_a_match.sql` afterwards; without it, Vercel crons call `close_ended_listings()`, `close_ended_matches()` and `close_ended_bounties()` through the service-role client (`/cron/close-listings`, `/cron/close-matches`, `/cron/close-bounties`) — the functions are the contract, the scheduler is interchangeable.
...
4. If pg_cron is not enabled, add a fourth variable, `CRON_SECRET` (any long random string). `vercel.json` schedules `GET /cron/close-listings`, `GET /cron/close-matches` and `GET /cron/close-bounties` every minute; Vercel sends the secret as `Authorization: Bearer …`, and the routes call `close_ended_listings()`, `close_ended_matches()` and `close_ended_bounties()` through the service-role client. Without the variable the routes answer 401 to every call, so they are harmless when pg_cron is doing the job.
