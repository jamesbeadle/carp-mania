...
{#if session.phase === 'tackle_up' && session.swim}
	<TackleBuilder {lake} swim={session.swim} overallSkill={session.overallSkill} onReady={(setups) => tackleUp(session, setups)} />
	...
	<div class="grid gap-6 lg:grid-cols-[3fr_2fr]">
		...
		<div>
			...
			{:else if session.phase === 'fishing'}
				<SessionHud {session} onStrike={() => strike(session)} />
			{:else}
