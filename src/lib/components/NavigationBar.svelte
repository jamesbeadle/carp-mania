<script lang="ts">
	...
	import InboxBadge from './inbox/InboxBadge.svelte';
...
<header class="border-b border-carbon-700 bg-carbon-950/90 backdrop-blur">
	<nav class="mx-auto flex max-w-6xl flex-wrap items-center gap-1 px-4 py-3">
		<a href="/home" class="mr-5"><Wordmark /></a>
		{#each links as link (link.href)}
			<a
				href={link.href}
				class="inline-flex items-center gap-1.5 rounded-md px-3 py-1.5 font-display text-base font-bold tracking-wide uppercase transition hover:text-volt-400"
				class:text-volt-500={isCurrent(link.href)}
				class:text-mist-200={!isCurrent(link.href)}
				class:border-b-2={isCurrent(link.href)}
				class:border-volt-500={isCurrent(link.href)}
			>
				{link.label}
				{#if link.href === InboxPath}<InboxBadge count={unreadCount} />{/if}
			</a>
		{/each}
