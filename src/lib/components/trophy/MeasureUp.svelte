...
<section class="panel border-surge-500/40">
	...
	<table class="w-full text-sm">
		<thead class="stat-label text-left"><tr><th class="py-1 font-normal"></th><th class="py-1 pr-2 font-normal"><span class="block max-w-32 truncate sm:max-w-none">{measureUp.theirName}</span></th><th class="py-1 font-normal">You</th><th class="py-1 font-normal"></th></tr></thead>
		<tbody class="divide-y divide-carbon-700/60">
			{#each rows as measure (measure.label)}
				<tr>
					<td class="py-1.5 pr-2 text-mist-400">{measure.label}</td>
					<td class="py-1.5 pr-2 whitespace-nowrap text-mist-100">{measure.theirs}</td>
					<td class="py-1.5 pr-2 whitespace-nowrap text-mist-100">{measure.yours}</td>
