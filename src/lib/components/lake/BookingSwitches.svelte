...
<form method="POST" action="?/sellSyndicate" class="mt-3 flex flex-wrap items-end gap-2 text-sm text-mist-200">
	...
	<label class="block text-xs text-mist-400">Places<input name="places" type="number" min="0" max="500" value={lake.syndicate_places_for_sale} class="field mt-1 w-24" /></label>
	<label class="block text-xs text-mist-400">Price a year (£)<input name="price" type="number" min="0" max="50000" value={Number(lake.syndicate_price)} class="field mt-1 w-28" /></label>
