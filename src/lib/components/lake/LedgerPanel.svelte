...
<div class="grid gap-6 lg:grid-cols-2">
	<section class="panel">
		<h3 class="mb-3 text-xl text-gold-300">Catch reports</h3>
		<CatchReportList {catches} {carpNames} />
	</section>
	<section class="panel">
		<h3 class="mb-3 text-xl text-gold-300">Day tickets</h3>
		{#if visits.length === 0}
			<p class="text-sm text-mist-400">Nobody has fished here yet.</p>
		{:else}
			<ul class="divide-y divide-pond-700/60 text-sm">
				{#each visits as visit (visit.id)}
					<li class="flex items-baseline gap-3 py-2">
						...
						<span class="ml-auto" class:text-gold-300={Number(visit.fee_paid) > 0} class:text-danger-400={Number(visit.fee_paid) === 0}>
