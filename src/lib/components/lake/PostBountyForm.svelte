...
<form method="POST" action="?/postBounty" class="space-y-3">
	...
	<div class="flex flex-wrap items-end gap-3">
		<label class="block text-xs text-mist-400">What wins it
		...
		{#if isNamed}
			<label class="block text-xs text-mist-400">Which fish
				<select name="targetCarpId" class="field mt-1">
					{#each namedFish as fish (fish.id)}<option value={fish.id}>{fishWords(fish)}</option>{/each}
				</select>
			</label>
		{/if}
		<label class="block text-xs text-mist-400">Prize £<input name="money" type="number" min={OwnersBounty.LeastMoney} max={OwnersBounty.MostMoney} step="50" bind:value={money} class="field mt-1 w-28" /></label>
		<label class="block text-xs text-mist-400">Days<input name="days" type="number" min={BountyDraw.ShortestWindowDays} max={BountyDraw.LongestWindowDays} bind:value={days} class="field mt-1 w-20" /></label>
