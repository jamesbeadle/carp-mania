...
<div class="space-y-3 border-t border-carbon-700 pt-3">
	<p class="font-display text-lg font-bold uppercase text-mist-100">{swim.name}</p>
	<form method="POST" action="?/renameSwim" class="flex gap-2">
		<input type="hidden" name="swimId" value={swim.id} />
		<input name="name" class="field" value={swim.name} minlength="2" maxlength="30" aria-label="Swim name" />
		<button class="button-secondary px-3 text-base">Rename</button>
	</form>
	{#if movePoint}
		<form method="POST" action="?/moveSwim" class="space-y-1">
			<input type="hidden" name="swimId" value={swim.id} />
			...
			<button class="button-primary w-full px-2 text-base" disabled={moveRefusal !== null}>Move here {formatMoney(SwimRules.MoveCost)}</button>
		</form>
	{:else}
		<p class="text-xs text-mist-400">Click or drag to a spot on the bank to move it for {formatMoney(SwimRules.MoveCost)}.</p>
	{/if}
	<form method="POST" action="?/removeSwim">
		<input type="hidden" name="swimId" value={swim.id} />
		<button class="button-secondary w-full px-2 text-base">Take out {formatMoney(SwimRules.RemoveCost)}</button>
	</form>
</div>
