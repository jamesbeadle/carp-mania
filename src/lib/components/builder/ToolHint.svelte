<script lang="ts">
	...
	import { forTheFinger } from '$lib/game/stage/pointerWords';
...
<div class="mt-2 flex items-center gap-3 lg:hidden">
	<p class="min-w-0 flex-1 text-sm text-mist-200"><span class="font-display font-bold tracking-wide text-volt-300 uppercase">{tool.label}</span> · {forTheFinger(builder.notice ?? tool.hint)}</p>
