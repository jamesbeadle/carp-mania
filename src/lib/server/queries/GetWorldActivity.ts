...
export async function GetWorldActivity(locals: App.Locals, limit: number): Promise<WorldActivity[]> {
	requireUser(locals);
	const { data: rows } = await locals.supabase.from('world_events').select(EventWithLakes).order('created_at', { ascending: false }).limit(limit);
