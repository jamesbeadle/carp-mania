...
export async function GetAnglerProfile(locals: App.Locals): Promise<AnglerProfile> {
	...
	const [{ data: catches }, { count }, { data: heaviest }, trophies] = await Promise.all([
	...
	return { profile, diary, catches: history, carpNames, lakeNames, personalBestLb: heaviest ? Number(heaviest.weight_lb) : 0, totalCatches: count ?? 0, trophies };
}
