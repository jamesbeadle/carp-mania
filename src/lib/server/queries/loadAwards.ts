...
type TallyRow = {
	best_lb: number; fish_landed: number; regions_fished: number; home_grown_best_lb: number; light_rod_best_lb: number; trophies: number;
	...
	fish_sold: number; bounties_taken: number;
};
...
export async function loadAwardTallies(supabase: SupabaseClient, anglerId: string): Promise<AwardTallies> {
	...
	const { fish_landed: fishLanded, regions_fished: regionsFished, trophies, records_set: recordsSet, record_waters: recordWaters } = row;
	const { night_fish: nightFish, morning_fish: morningFish, ticket_kinds_fished: ticketKindsFished, fish_sold: fishSold, bounties_taken: bountiesTaken } = row;
	return {
		bestLb: Number(row.best_lb), fishLanded, regionsFished, homeGrownBestLb: Number(row.home_grown_best_lb),
		lightRodBestLb: Number(row.light_rod_best_lb), trophies, recordsSet, recordWaters,
		lifetimeLb: Number(row.lifetime_lb), nightFish, morningFish, ticketKindsFished, fishSold, bountiesTaken
	};
}
