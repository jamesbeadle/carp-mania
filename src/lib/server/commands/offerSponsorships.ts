...
export async function offerSponsorships(locals: App.Locals, trusted: SupabaseClient, lake: Lake, days: number, from: Date) {
	const [deal, offers] = await Promise.all([loadRunningDeal(locals, lake.id, from), loadOpenOffers(locals, lake.id, from)]);
	const random = seededRandom(new Date(lake.simulated_until).getTime() + OfferSalt);
	const drawn = offersOverDays(lake.id, waterRatingOfLake(lake), days, { deal, offers }, random, from);
