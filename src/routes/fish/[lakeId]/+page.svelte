...
<h1 class="mb-1 text-4xl text-gold-300">{data.water.lake.name}</h1>
<p class="mb-6 text-sm text-mist-400">{data.water.ownerName}'s water · reputation {Math.round(Number(data.water.lake.reputation))} · {data.water.carp.length} carp</p>

{#if data.visitId}
	<FishingGame lake={data.water.lake} swims={data.water.swims} carp={data.water.carp} profile={data.profile} visitId={data.visitId} />
{:else}
	<section class="panel max-w-xl">
		<h2 class="mb-2 text-2xl text-gold-300">Day ticket</h2>
